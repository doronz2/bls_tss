...
use curv::cryptographic_primitives::pairing::pairing_bls12_381::{PairingBls, PAIRING};
...
pub fn compute_signature<T: ECPoint + Clone + Debug, P: ECPoint>(
    key_pair: &KeyPair<P>,
    message: &BigInt,
) -> T {
...
pub fn verify_bls_signature(key_pair: &KeyPair<GE2>, message: &BigInt) -> bool {
    ...
    let right_side = PairingBls::compute_pairing(
        &compute_signature::<GE1, GE2>(key_pair, message),
        &GE2::generator(),
    );
